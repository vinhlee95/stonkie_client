    ...
    <div className="overflow-x-auto">
      <table className="min-w-full bg-white dark:bg-black border border-gray-300 dark:border-gray-700">
        <thead>
          <tr>
            ...
            {revenueData.map((yearData) => (
              <th
                key={yearData.year}
                className="px-4 py-2 text-right border-b border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-[var(--dark-background)]"
              >
                {yearData.year}
              </th>
            ))}
        ...
        <tbody>
          {Array.from(
            new Set(
              revenueData.flatMap((yearData) => yearData.breakdown.map((item) => item.label)),
            ),
          )
            .sort()
            .map((product, rowIndex) => (
              <tr
                ...
                className={
                  rowIndex % 2 === 0
                    ? 'bg-gray-50 dark:bg-[var(--dark-background)]'
                    : 'bg-white dark:bg-[var(--dark-background)]'
                }
              >
                ...
                {revenueData.map((yearData) => {
                  ...
                  return (
                    <td
                      key={yearData.year}
                      className="px-4 py-2 text-right border-b border-gray-300 dark:border-gray-700"
                    >
...
export default async function Revenue({ params }: { params: Promise<{ ticker: string }> }) {
  ...
  const res = await fetch(
    `${process.env.BACKEND_URL}/api/companies/${ticker.toLowerCase()}/revenue`,
    {
  ...
  const productRevenueData = revenueData
    .map((data) => ({
      year: data.year,
  ...
  const regionRevenueData = revenueData
    .map((data) => ({
      year: data.year,
  ...
  return (
    <div className="p-4">
      ...
      <h1 className="text-2xl font-bold mb-4 mt-4">
        Region revenue breakdown: {ticker.toUpperCase()}
      </h1>
