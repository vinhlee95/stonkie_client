...
import { AnnualFinancialStatement, QuarterlyFinancialStatement, FinancialStatement, isAnnualStatement, isQuarterlyStatement } from "@/app/types";
import { ChartDataOutput, ChartDatasetDefinition, processAnnualStatements, processQuarterlyStatements } from "./chartUtils";

// Helper function similar to getChartDataset in GrowthChart.tsx
function getEpsChartDataset(
...
export default async function EpsChart({ticker, incomeStatements: annualStatements}: {ticker: string, incomeStatements: AnnualFinancialStatement[]}) {
  ...
  if ((!annualData.datasets || annualData.datasets.length === 0) && (!quarterlyData.datasets || quarterlyData.datasets.length === 0)) {
    return null;
  }
