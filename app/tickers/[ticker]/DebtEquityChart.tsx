...
export function getChartDataset(
  balanceSheets: Array<FinancialStatement>): ChartDataOutput {
  ...
  const equityKey = Object.keys(firstData).find(key =>
    key.toLowerCase().includes('total common equity') || key.toLowerCase().includes('common stock equity')
  );
  ...
  const totalDebt = sortedBalanceSheets.map(key =>
    parseFinancialValue(key.data[totalDebtKey!]) * 1000
  )

  const equity = sortedBalanceSheets.map(key =>
    parseFinancialValue(key.data[equityKey!]) * 1000
  );
...
export default async function DebtEquityChart({balanceSheet: annualBalanceSheet, ticker}: {balanceSheet: AnnualFinancialStatement[], ticker: string}) {
  ...
  return (
    <ChartWithPeriod
      ...
      quaterlyDatasets={quarterlyData.datasets} // quaterlyDatasets❌ quarterlyDatasets ✅
