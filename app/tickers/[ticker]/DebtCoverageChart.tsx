...
function getChartDataset(
  ...
  if (isAnnualStatement(balanceSheets[0] ?? cashFlows[0])) {
    ...
    const { sortedStatements: annualSortedCF } = processAnnualStatements(
      cashFlows as AnnualFinancialStatement[],
    )
    ...
    const { sortedStatements: quarterlySortedCF } = processQuarterlyStatements(
      cashFlows as QuarterlyFinancialStatement[],
    )
    ...
    console.error(
      'Unknown or mixed statement types in getChartDataset',
      balanceSheets[0] ?? cashFlows[0],
    )
    return { labels: [], datasets: [] }
  }
  ...
  const cashKey = Object.keys(firstBalanceSheetData).find((key) => key.toLowerCase() === 'cash')
  const cashEquivalentsKey = Object.keys(firstBalanceSheetData).find(
    (key) => key.toLowerCase() === 'cash equivalents',
  )
  ...
  const datasets: ChartDatasetDefinition[] = [
    {
      type: 'bar' as const,
      label: 'Total Debt',
      data: sortedBalanceSheets.map(
        (statement) => parseFinancialValue(statement.data[totalDebtKey!]) * 1000,
      ),
    ...
    },
    {
      ...
      data: sortedCashFlows.map(
        (statement) => parseFinancialValue(statement.data[freeCashFlowKey!]) * 1000,
      ),
    ...
    },
    {
      ...
      data: sortedBalanceSheets.map((statement) => {
        ...
        if (
          cashKey &&
          cashEquivalentsKey &&
          statement.data[cashKey] &&
          statement.data[cashEquivalentsKey]
        ) {
...
export default async function DebtCoverageChart({
