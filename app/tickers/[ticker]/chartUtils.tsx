...
export interface ChartDatasetDefinition {
  // Renamed to avoid conflict if ChartDataset is a global type
  type: 'bar' | 'line'
  label: string
  data: number[]
...
export function processAnnualStatements(statements: AnnualFinancialStatement[]): {
  ...
  const generatedLabels = sortedAnnual.map((s) => (s.is_ttm ? 'TTM' : s.period_end_year.toString()))
...
export function processQuarterlyStatements(statements: QuarterlyFinancialStatement[]): {
  ...
  const sortedQuarterly = [...statements].sort(
    (a, b) => new Date(a.period_end_quarter).getTime() - new Date(b.period_end_quarter).getTime(),
  )
